...
* **`train/loss`:**
    * 学習の不安定さを確認します。あまりに激しく乱高下している場合は、学習率（learning_rate）が高すぎるか、物理モデル（kv値など）に無理がある可能性があります。

### グラフが動かない・表示されない場合
1.  **保存先の確認:** スクリプト内で `tensorboard_log="./ppo_1/"` のように指定されているか確認してください。
2.  **フォルダの中身:** `ppo_1` フォルダの中に、`PPO_1`, `PPO_2`...といったサブフォルダがあり、その中に `events.out.tfevents...` という名前のファイルが入っている必要があります。
3.  **再読み込み:** TensorBoardの画面右上にある更新ボタン（Settingsの横）を押すと、最新の学習状況が反映されます。



もし「飛び跳ねる」挙動が続いている場合、`ep_rew_mean` がある程度のところで頭打ちになり、それ以上改善しない傾向が見られるはずです。その際は、前回お伝えした物理パラメータ（kvやforcerange）の調整に戻る判断材料になります。
